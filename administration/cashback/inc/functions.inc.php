...
if (!function_exists('DisplayCashback')) {
	function DisplayCashback($value)
	{
		...
		{
            $cashback = 'EUR'.number_format($value, 2, ',', '');
		}
...
if (!function_exists('GetTimeLeft')) {
	function GetTimeLeft($time_left)
	{
		...
		$days == 1 ? $dw = 'jour' : $dw = 'jours';
		$hours == 1 ? $hw = 'heure' : $hw = 'heures';
		$minutes == 1 ? $mw = 'minute' : $mw = 'minutes';
		$seconds == 1 ? $sw = 'seconde' : $sw = 'secondes';
		...
		{
			return "<span class='expired'>expiré</span>";
		}
	}
}
...
if (!function_exists('GetUsername')) {
	function GetUsername($user_id, $hide_lastname = 0)
	{
		...
		$result = smart_mysql_query("SELECT * FROM users WHERE id='".(int)$user_id."' LIMIT 1");

		if (mysqli_num_rows($result) != 0)
		{
			$row = mysqli_fetch_array($result);
			if ($hide_lastname == 1)
				return $row['nom']." ".substr($row['prenom'], 0, 1).".";
			else
	...
	function ShowCategories($cat_id, $level=0)
	{
		...
		if (mysqli_num_rows($result) >= 1)
		{
			while ($row = mysqli_fetch_array($result))
			{
				...
				if (isset($_GET['cat']) && $_GET['cat'] === $row['category_id']) $actives = " class=\"active\""; else $actives = "";
				...
				{
					if (isset($_GET['cat']) && (isParent($_GET['cat']) || isParent2($_GET['cat']))) ShowCategories($row['category_id'], $level+1);
				}
				else
				{
